...
## Skillify — 经验→技能 (Experience to Skill)

Scan learnings and suggest patterns worth codifying into reusable skills or checklists.
...
').filter(Boolean);
    ...
    for (const line of lines) {
      try {
        const e = JSON.parse(line);
        const dk = e.key || 'unknown';
        if (!byKey[dk]) byKey[dk] = [];
        byKey[dk].push(e);
      } catch {}
    }
    // Find keys that appear 3+ times (frequent patterns)
    const candidates = Object.entries(byKey)
      .filter(([k, v]) => v.length >= 3)
      .map(([k, v]) => ({ key: k, count: v.length, types: [...new Set(v.map(x => x.type))], latest: v.sort((a,b) => new Date(b.ts||0) - new Date(a.ts||0))[0] }));
...
Analyze the output and suggest skill-worthy patterns to the user:

**Criteria for skill-worthiness:**
- Same learning key appears 3+ times → stable pattern worth codifying
- High confidence (8-10) + practical impact → strong skill candidate
- Pairs well with an existing gstack skill (e.g., a checklist for /review)

**Output format:**

```
## Skill-worthy patterns from learnings

### [key] (appeared N times)
- Type: [type]
- Insight: [insight]
- Suggestion: Turn this into a [checklist / review gate / prompt template]
- Natural fit: Add to /[existing-skill] or create as new standalone skill
```

For each candidate, ask the user:

> "**[key]** has appeared [N] times. It could be codified as a **[suggestion]**.
> A) Create as a new skill file
> B) Propose as an addition to /[existing-skill]
> C) Skip — not worth codifying"

**Important:** Only suggest genuine multi-occurrence patterns. Do NOT suggest skillifying
single-occurrence or low-confidence entries.
