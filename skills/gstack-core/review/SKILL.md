...
## Step 3.75: Context completeness self-check

Before running the critical pass, verify you have enough context to do a proper review.
The 6 most common review failures come from missing context:

   ```
   [ ] 代码结构不符: Does the code follow existing project structure?
   [ ] 命名风格一致: Do names match project convention?
   [ ] 异常处理: Are errors handled consistently?
   [ ] 权限检查: Does this respect existing auth boundaries?
   [ ] 事务一致性: Does the change preserve data integrity?
   [ ] Demo代码: Is this production-ready or a demo snippet?
   ```

If any item is unclear, use Grep/Read to gather more context before proceeding.
**Do NOT flag issues based on assumptions.**

---

### 代码审查三层次框架（文章第七节）

文章指出代码审查应该覆盖三个层次，Step ④ 依次执行：
...
Also apply the remaining INFORMATIONAL categories that are still in the checklist.

**Article-derived专项检查 (AI输出质量):** Beyond the checklist, inspect the diff for these
common AI-generated code issues that human reviewers often miss:

   🔴 异常处理一致性
   - Every new function: does it have proper error handling?
   - async functions: are they wrapped in try-catch or chained with .catch()?
   - Error messages: do they propagate to the user, or get silently swallowed?

   🔴 权限边界
   - Does the change introduce new API endpoints without auth checks?
   - Are existing auth guards (middleware, decorators) still applied?
   - If roles are involved, are all new paths authorized for the correct roles?

   🔴 事务与数据一致性
   - If the change touches the database, are writes wrapped in transactions?
   - Partial failure scenarios: if step 2 of 3 fails, does step 1 roll back?
   - Concurrent access: can two requests interleave and corrupt state?

   🔴 边界值与空值
   - What happens when the input is empty/null/zero?
   - What happens when the result set is empty?
   ...
   - Is there commented-out code that should be removed? (Async/Sync Mixing, Column/Field Name Safety, LLM Prompt Issues, Type Coercion, View/Frontend, Time Window Safety, Completeness Gaps, Distribution & CI/CD).
