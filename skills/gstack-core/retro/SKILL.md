...
After the main retro output, extract actionable insights for future sessions:

### 1. Log key findings to learnings

For each significant finding (new pattern, recurring pitfall, architectural decision),
log it so future sessions can find it:

```bash
eval "$(~/.claude/skills/gstack/bin/gstack-slug 2>/dev/null)"
~/.claude/skills/gstack/bin/gstack-learnings-log '{"skill":"retro","type":"pattern","key":"KEY","insight":"INSIGHT","confidence":N,"source":"observed","files":["FILE"]}'
```

Good candidates for learnings:
- A new coding pattern or approach discovered this week
- A recurring issue that suggests a systemic problem
- A decision rationale that should be remembered

### 2. Detect skill-worthy patterns

If the same type of finding appears across multiple weeks, it may be worth codifying
as a skill. Signal to the user:

> "I notice [finding X] has appeared in [N] retros this month. This might be worth
> turning into a reusable skill or checklist. Run `/learn skillify` to explore this."

### 3. Update trend data

If this retro introduced a new metric or changed a trend direction, note it in
the `.context/retros/` snapshot alongside the existing metrics.
