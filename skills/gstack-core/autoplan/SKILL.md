...
7. **Section 3.6 (Task Granularity — 小步粒度检查):** Review the task breakdown for proper granularity.
   Each task should be independently implementable, testable, and verifiable.

   **粒度规则:**
   - ✅ Good: A single function/component with clear inputs and outputs (< 100 lines change)
   - ✅ Good: A self-contained module change with its own test file
   - ❌ Too large: "Implement the entire auth system" — break into: login form, token management, middleware, tests
   - ❌ Too large: A task that touches >5 files or spans >3 modules
   - ❌ Missing: Tasks without a corresponding test task or testing step
   - ❌ Missing: Tasks without explicit acceptance criteria

   Flag any oversized tasks and suggest a proper breakdown. Auto-decide the split
   using principle P5 (explicit > clever) — smaller tasks with clear boundaries
   are always better than larger ambiguous ones.

8. **Section 3.7 (Process Stage Completeness):** Verify the plan includes all required stages
   for each task or task group:

   ```
   [ ] Implementation — the code change
   [ ] Testing — unit/integration tests for the change
   [ ] Documentation — if it changes UI, API, or architecture
   [ ] Review — who reviews what
   ```

   If any stage is missing, flag it. Auto-decide using P1 (evals always) — testing and
   documentation are not optional stages.
...
- **小步粒度原则:** Every task in the plan must be independently implementable, testable, and verifiable. If a task is too large to complete in a single focused session (< 100 lines effective change), split it. Tasks should have clear acceptance criteria and include test/doc/review stages.
