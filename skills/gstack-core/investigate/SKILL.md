...
> **文章指出：报错排查是 AI 编程中最能体现价值的地方。**
> 如果你直接把错误信息贴进去，效果会好很多。
> 不要自己先猜原因，让 AI 从完整错误信息开始分析。
...
0. **Context completeness check:** Before diving in, verify you have enough information.
   Run through this checklist and mark each item:

   ```
   [ ] Error message / full stack trace — provided?
   [ ] Reproduction steps — clear and deterministic?
   [ ] Affected code / files — visible?
   [ ] Input data or parameters — what was passed?
   [ ] Environment — dev/staging/prod? Browser/OS/versions?
   [ ] Recent git changes — in the affected area?
   ```

   If any item is missing, use AskUserQuestion to request it — ONE question at a time.
   **Do NOT guess missing context.** Every assumption you make without evidence is a potential wrong turn.
...
6. **Iterative Q&A (连环追问):** If the bug pattern is still unclear after steps 1-5,
   start an iterative Q&A loop with the user — ask ONE question at a time,
   and wait for the answer before asking the next.

   Good probing questions to ask (one at a time):

   ```
   - "Can you share the complete error message / full stack trace?"
   - "What were the exact inputs or operations that triggered this?"
   - "Is this bug deterministic (always happens) or intermittent?"
   - "Does it only happen under certain conditions (e.g., specific data, time, load)?"
   - "Were there any recent code changes or deployments before this started?"
   - "Can you reproduce it in a local dev environment, or only in staging/production?"
   ```

   Each answer narrows the search space. Do NOT batch multiple questions into one message.
   Let the user's answer guide your next question. Avoid yes/no questions when possible —
   prefer "what" and "how" questions that produce richer information.
...
- **连环追问原则:** If context is insufficient, ask ONE question at a time, let the answer guide the next. Never batch multiple questions. A single well-placed question can save 3 wrong guesses.
- **Completion status:**
