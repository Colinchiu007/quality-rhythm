...
## Phase 2.8: 需求边界探测 (Requirement Boundary Probe)

After the user's problem is clear, probe for blind spots before moving to premise challenge.
Ask yourself (and the user, one question at a time) about these common boundary gaps:

   ```
   [ ] 需求盲区: Does the user's description assume a solution instead of stating the problem?
   [ ] 边界情况: What happens at zero/empty/maximum inputs?
   [ ] 异常路径: What should happen when something goes wrong?
   [ ] 用户细分: Is this for all users, or a specific subset?
   [ ] 隐性依赖: What other systems/teams/data does this depend on?
   [ ] 不做清单: What is explicitly OUT of scope?
   ```

Use AskUserQuestion to probe any gaps found — ONE question at a time.
Probing questions that work well:

   - "You mentioned [X] — what does success look like specifically?"
   - "What would happen if [Y] went wrong?"
   - "Who is this NOT for?"
   - "If you could only do 20%% of this, what would be the most important piece?"

**Important:** Do NOT skip this phase even if the user has a "simple" idea.
Most requirement failures come from assumptions that felt obvious at the time.
