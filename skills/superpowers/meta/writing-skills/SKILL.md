...
**Purpose:** Codex reads when_to_use to decide which skills to load for a given task. Make it answer: "Should I read this skill right now?"
...
## Extracting Skills from Learnings

A skill is codified experience. If you have a collection of learnings
(patterns, pitfalls, preferences), you can distill them into a reusable skill.

### The Extraction Process

1. **Gather:** Collect learnings from the same domain (e.g., all "python-migration" tagged entries)
2. **Cluster:** Group by theme — what patterns are repeated? What pitfalls keep appearing?
3. **Generalize:** Extract the common principle — if 5 learnings say "don't use bare except", the principle is "always catch specific exceptions"
4. **Codify:** Write as a SKILL.md with:
   - `when_to_use` based on the triggering conditions from the learnings
   - Rules derived from the generalized patterns
   - Anti-patterns from the pitfalls
5. **Test:** Use this skill"s TDD process to verify the new skill works

### Signal Detection

A learning is worth extracting into a skill when:

- **Frequency:** Same pattern appears 3+ times across different sessions
- **Impact:** Following the pattern consistently would prevent real bugs
- **Teachability:** The pattern can be explained as a simple rule or checklist
- **Triggerability:** You can describe when to apply it ("when you see X, do Y")

### From Retro to Skill

Retrospectives are the richest source of skill candidates.
After each retro, scan the findings for:

- "This keeps happening" → recurring pitfalls → skill check
- "We should always X" → process rules → skill rule
- "Nobody knew Y" → knowledge gaps → skill reference
