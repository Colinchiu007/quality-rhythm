...
A task is "right-sized" when it can be implemented, tested, and verified in a single focused session.
Apply these rules when breaking work into subagent tasks:

### ✅ Right-sized task
- Changes 1-3 files
- Has clear acceptance criteria ("what done looks like")
- Can be tested independently
- Takes 15-60 minutes for a skilled developer

### ❌ Too large (must split)
- Spans >5 files or >3 modules
- Has ambiguous success criteria
- Requires multiple architectural decisions
- Would take >2 hours in one pass

### ❌ Too small (should combine)
- Single line change with no test needed
- Trivial rename that doesn't affect behavior
- Mechanical change with zero logic

### The "Independent Module" Rule

Each task should map to a single logical module or concern. If a task description
contains "and" ("implement login AND dashboard"), split it. If it requires
knowledge of two unrelated subsystems, split it.

Tasks should be **independent enough that a fresh subagent can complete them
without reading other tasks' implementations**.
